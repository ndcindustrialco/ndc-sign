...
function SignerStatusBadge({ status }: { status: string }) {
  return (
    <span
      className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${SIGNER_STATUS_STYLES[status] ?? SIGNER_STATUS_STYLES.PENDING}`}
    >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      ...
      >
        ...
        <p className="mb-4 text-sm" style={{ color: "var(--accent, #ADB5BD)" }}>
          จะยกเลิกการลงนามที่รอทั้งหมดของ{" "}
          <strong style={{ color: "var(--foreground, #212529)" }}>
            {documentName}
          </strong>{" "}
          และแจ้งผู้ลงนาม
        </p>
    ...
    <div className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          >
            สร้างเมื่อ Created {formatDate(doc.createdAt)} · {totalSigners}{" "}
            ผู้ลงนาม signer{totalSigners !== 1 ? "s" : ""}
          </p>
        ...
        <div className="flex items-center gap-2">
          {(signedPdfUrl ?? pdfUrl) && (
            <a
              href={
                doc.status === "COMPLETED" && signedPdfUrl
                  ? signedPdfUrl
                  : (pdfUrl ?? "")
              }
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-lg px-3 py-2 text-xs font-medium transition-colors hover:bg-slate-100"
              style={{
                border: "1px solid var(--border, #E5E7EB)",
                color: "var(--foreground, #212529)",
              }}
          ...
          {auditPdfUrl && doc.status === "COMPLETED" && (
            <a
              href={auditPdfUrl}
              target="_blank"
              rel="noopener noreferrer"
          ...
          {canRemind && (
            ...
            >
              {remindingAll
                ? "กำลังส่ง…"
                : `แจ้งเตือนทั้งหมด Remind all (${pendingSignerCount})`}
            </button>
          )}
          {canVoid && (
            <button
                            ...
                            <td className="px-4 py-3 text-right">
                              {canRemindThis && (
                                <button
                                  ...
                                  disabled={
                                    isPending || remindingId === signer.id
                                  }
        ...
        <div className="space-y-4">
          ...
          {totalSigners > 0 && doc.status !== "DRAFT" && (
            ...
            >
              ...
              >
                <span>
                  {signedCount} จาก {totalSigners} ลงนามแล้ว signed
                </span>
