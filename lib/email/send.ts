...
export async function sendSigningInvite(opts: {
  ...
  await sendGraphMail({
    ...
    subject: opts.customSubject ?? `Please sign: ${opts.documentName}`,
    htmlBody: html,
...
export async function sendSignedNotification(opts: {
  ...
  await sendGraphMail({
    ...
    subject: `${opts.signerName} has signed "${opts.documentName}"`,
    htmlBody: html,
...
export async function sendCompletedNotification(opts: {
  ...
  await sendGraphMail({
    ...
    fromEmail: opts.ownerEmail,
    fromName: opts.ownerName,
    subject: `All signatures collected — "${opts.documentName}"`,
    htmlBody: html,
    toAddress: opts.ownerEmail,
    toName: opts.ownerName,
...
export async function sendSignerCopy(opts: {
  ...
  await sendGraphMail({
    ...
    fromEmail: opts.ownerEmail,
    fromName: opts.ownerName,
    subject: `Your signed copy — "${opts.documentName}"`,
    htmlBody: html,
    toAddress: opts.signerEmail,
    toName: opts.signerName,
    attachments: attachments.length > 0 ? attachments : undefined,
...
export async function sendVoidNotification(opts: {
  ...
  await sendGraphMail({
    ...
    fromEmail: opts.ownerEmail,
    fromName: opts.ownerName,
    subject: `Signing request cancelled — "${opts.documentName}"`,
    htmlBody: html,
    toAddress: opts.signerEmail,
    toName: opts.signerName,
  })
...
export async function sendDeclineNotification(opts: {
  ...
  await sendGraphMail({
    ...
    fromEmail: opts.ownerEmail,
    fromName: opts.ownerName,
    subject: `${opts.signerName} declined to sign "${opts.documentName}"`,
    htmlBody: html,
    toAddress: opts.ownerEmail,
    toName: opts.ownerName,
  })
